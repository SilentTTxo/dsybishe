		...
		$.ajax({
			...
			success : function(result){
				...
				if(result['code'] == "1"){
					...
					user = result['user'];
					for(i in user){
						$("tbody").append('<tr><td>{id}</td><td>{username}</td><td>{password}</td><td>{money}</td><td>{img}</td><td><select class="small m-wrap" tabindex="1" class="power" id="s{id}" dataid="{id}"><option value="-1">普通用户</option><option value="0">管理员</option></select></td></tr>'.format(user[i]));
						$("#s{id}".format(user[i])).val(user[i].power);
						$("#s{id}".format(user[i])).change(function(){
							g={}
							g.id = $(this).attr("dataid");
							g.power = $(this).val();
							$.ajax({
								type:"GET",
								url:"./api/user/fixPower?id={id}&power={power}".format(g),
								success : function(result){
									showMsg("修改成功");
								}
							})
