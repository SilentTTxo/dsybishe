		...
		$.ajax({
			...
			success : function(result){
				...
				if(result['code'] == "1"){
					...
					for(i in goods){
						if(goods[i].isSell == 1){
							goods[i].isSell = '<td><span class="label label-success">在架</span></td>'
						}
						if(goods[i].isSell == 0){
							goods[i].isSell = '<td><span class="label label-info">下架</span></td>'
						}
						$("tbody").append('<tr><td>{id}</td><td>{name}</td><td>{price}</td><td>{goodstype}</td><td>{address}</td><td>{sex}</td><td>{age}</td><td>{tel}</td><td>{des}</td>{isSell}<td>{img}</td><td><a href="javascript:" class="btn mini black del" data="{id}"><i class="icon-trash"></i>删除</a></td></tr></tr>'.format(goods[i]));
					}
					$(".del").click(function(){
						$.ajax({
							type:"GET",
							url:"./api/goods/del?id="+$(this).attr("data"),
							success : function(result){
								result = jQuery.parseJSON(result);
								if(result['code'] == "1"){
									showMsg("删除成功");
