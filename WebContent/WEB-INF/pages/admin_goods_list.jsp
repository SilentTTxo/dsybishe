			...
			success : function(result){
				...
				if(result['code'] == "1"){
					...
					for(i in goods){
						...
						$("tbody").append('<tr><td>{id}</td><td>{name}</td><td>{price}</td><td>{goodstype}</td><td>{address}</td><td>{sex}</td><td>{age}</td><td>{tel}</td><td>{des}</td>{isSell}<td>{img}</td></tr>'.format(goods[i]));
					}
				}
				else{
					showMsg("系统故障")
